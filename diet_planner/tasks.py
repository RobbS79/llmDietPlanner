            ...
            try:
                ...
                def calculate_item_total_price(item: Dict[str, Any]) -> Optional[Decimal]:
                    ...
                    if package_size and package_size > 0 and package_size != 1:
                        ...
                        logger.info(f"  Package detected: {offer_price} for {package_size} {norm_offer_unit}, per-unit: {per_unit_price} {norm_offer_unit}")
                    else:
                    ...
                    if norm_quantity_unit == norm_offer_unit:
                        ...
                        # Special check for pieces: if price per piece seems too high (> 10 CZK per piece) 
                        # and we need multiple pieces, it's likely a package price we didn't detect
                        # Lowered threshold to 10 CZK to catch more cases (eggs are typically 5-8 CZK per piece)
                        if norm_quantity_unit == 'ks' and required_qty > 1 and per_unit_price > Decimal('10'):
