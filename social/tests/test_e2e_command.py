...
def _seams(decisions, **kw):
    seams = dict(
        ...
        today=TODAY, sleep=MagicMock(), stdout=io.StringIO(),
    )
    seams.update(kw)
    return seams


APPROVED = Decision('approved', 'UHUMAN')
...
class SocialE2ECommandTests(TestCase):
    def test_waits_for_approval_then_publishes_to_facebook_only_and_reads_back(self):
        # pending twice while polling, then ✅; publish_social_posts reads it once more
        seams = _seams([Decision('pending'), Decision('pending'), APPROVED, APPROVED])
        ...
        self.assertEqual(seams['sleep'].call_count, 2)
    ...
    def test_does_not_touch_other_due_drafts(self):
        ...
        seams = _seams([APPROVED, APPROVED])
        call_command('social_e2e', **seams)
    ...
    def test_timeout_rejects_the_draft_so_the_scheduled_job_never_posts_it(self):
        seams = _seams([Decision('pending')] * 3)
        with self.assertRaisesMessage(CommandError, 'no ✅'):
    ...
    def test_rejection_in_slack_fails_without_publishing(self):
        seams = _seams([Decision('rejected'), Decision('rejected')])
