      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-12 py-12">
        ...
        {isLoading ? (
          ...
          <div className="py-20 text-center">
            <ChefHat size={64} className="text-zinc-300 mx-auto mb-6" />
            <p className="text-zinc-300 text-sm font-bold italic">Zatím žádné veřejné recepty. Recepty se objeví, jakmile uživatelé začnou generovat jídelníčky.</p>
          </div>
        ) : (
          <>
            ...
            {data && data.num_pages > 1 && (
              <div className="flex items-center justify-center gap-4 mt-12">
                {data.has_previous && (
                  <button onClick={() => setPage(p => p - 1)} className="px-6 py-3 border border-slate-600 text-zinc-200 hover:text-white rounded-xl font-black uppercase text-[10px] tracking-widest transition-colors">
                    Předchozí
                  </button>
                )}
                <span className="text-zinc-300 text-xs font-bold">Strana {data.current_page} z {data.num_pages}</span>
                {data.has_next && (
                  <button onClick={() => setPage(p => p + 1)} className="px-6 py-3 border border-slate-600 text-zinc-200 hover:text-white rounded-xl font-black uppercase text-[10px] tracking-widest transition-colors">
                    Další
                  </button>
